...
class TrixV21Strategy(IStrategy):
    ...
    buy_trix_timeperiod = IntParameter(7, 13, default=12, space="entry", optimize=False, load=True)
    buy_trix_src = CategoricalParameter(['open', 'high', 'low', 'close'], default='close', space="entry", optimize=False, load=True)
    buy_trix_signal_timeperiod = IntParameter(19, 25, default=22, space="entry", optimize=False, load=True)
    buy_trix_signal_type = CategoricalParameter(['trailing', 'trigger'], default='trigger', space="entry", optimize=False, load=True)

    buy_ema_timeperiod_enabled = BooleanParameter(default=True, space="entry", optimize=False, load=True)
    ...
    buy_ema_guard_timeperiod = IntParameter(150, 250, default=200, space="entry", optimize=True, load=True)
    buy_ema_guard_multiplier = DecimalParameter(0.8, 1.0, decimals=3, default=0.97, space="entry", optimize=True, load=True)
    ...
    sell_trix_timeperiod = IntParameter(7, 11, default=9, space="exit", optimize=False, load=True)
    sell_trix_src = CategoricalParameter(['open', 'high', 'low', 'close'], default='close', space="exit", optimize=False, load=True)
    sell_trix_signal_timeperiod = IntParameter(17, 23, default=19, space="exit", optimize=False, load=True)
