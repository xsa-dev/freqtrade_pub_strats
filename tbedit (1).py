...
class tbedit(NostalgiaForInfinityNext):
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str, amount: float,
        ...
        if len(dataframe) >= 1:
            ...
            # custom sell
            if (sell_reason not in (
                    'stoploss_on_exchange', 'trailing_stop_loss', 'emergency_sell', 'stop_loss', 'roi',
                    'force_sell')) and (
                    last_candle['sell'] == 0) and (not self.trailing_on_custom_sell):
                return val
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.trailing_buy_order_enabled and self.abort_trailing_when_sell_signal_triggered and self.config[
            'runmode'].value in ('live', 'dry_run'):
            last_candle = dataframe.iloc[-1].squeeze()
